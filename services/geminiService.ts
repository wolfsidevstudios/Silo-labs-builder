...
import { getCredentials as getTwilioCredentials } from './twilioService';
import { getPublisherId as getGoogleAdsenseId } from './googleAdsenseService';
import { getMeasurementId as getGoogleAnalyticsId } from './googleAnalyticsService';
import { getApiKey as getTripoApiKey } from './tripoService';
...
---
`; }
function getTripoInstruction(): string { 
    if (!getTripoApiKey()) return ''; 
    return `
---
**TRIPO AI API AVAILABLE:**
A Tripo AI API key is available. If the user asks for 3D model generation, follow the Tripo AI API integration rules in the system prompt.
---
`; 
}
