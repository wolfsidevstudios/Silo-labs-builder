...
import { Secret, GitHubUser, GitHubRepo } from '../types';
import { getSecrets, addSecret, removeSecret, identifyTool, Tool } from '../services/secretsService';
import { savePat, getPat, removePat, getUserInfo, getRepositories } from '../services/githubService';
...
const SettingsPage: React.FC<SettingsPageProps> = ({ isPro, onUpgradeClick }) => {
  ...
  useEffect(() => {
    ...
    if (pat) {
        ...
        getUserInfo(pat)
            .then(user => {
                setGithubUser(user);
                return getRepositories(pat);
            })
            .then(repos => setGithubRepos(repos))
            .catch(() => {
                setGithubError("Your PAT seems to be invalid or expired. Please connect again.");
                removePat(); // Clean up invalid token
            })
  ...
  const handleConnectGitHub = async () => {
    if (!githubPat.trim()) {
        setGithubError("Please enter a Personal Access Token.");
        return;
    }
    setIsConnecting(true);
    setGithubError(null);
    try {
        const user = await getUserInfo(githubPat);
        ...
        setGithubError("Connection failed. Please check your token and permissions.");
    } finally {
    ...
    <div className="min-h-screen w-screen bg-black flex flex-col items-center p-4 pl-20 selection:bg-indigo-500 selection:text-white overflow-y-auto">
      <main className="w-full max-w-5xl px-4 py-12">
        ...
        <div className="mt-16 bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-2xl p-8 shadow-[0_0_120px_rgba(255,255,255,0.1)] animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          <h2 className="text-2xl font-bold text-slate-100 mb-4">Connected Apps</h2>
          {githubUser ? (
            <div>
              <div className="flex items-center justify-between bg-slate-800/50 p-4 rounded-lg">
                <div className="flex items-center gap-4">
                  <img src={githubUser.avatar_url} alt="GitHub avatar" className="w-12 h-12 rounded-full border-2 border-slate-600" />
                  <div>
                    <p className="font-bold text-lg text-white">{githubUser.name}</p>
                    <a href={githubUser.html_url} target="_blank" rel="noopener noreferrer" className="text-sm text-slate-400 hover:text-indigo-400">@{githubUser.login}</a>
                  </div>
                </div>
              ...
              <div className="mt-6">
                <h3 className="font-semibold text-slate-300 mb-3">Your Repositories</h3>
                <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
                  {githubRepos.map(repo => (
                    <a href={repo.html_url} target="_blank" rel="noopener noreferrer" key={repo.id} className="block bg-slate-900/50 hover:bg-slate-800 p-3 rounded-md transition-colors">
                      <p className="font-mono text-sm text-slate-200 truncate">{repo.name} {repo.private && <span className="text-xs bg-slate-700 px-1.5 py-0.5 rounded-sm ml-2">Private</span>}</p>
                    </a>
                  ))}
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="font-semibold text-slate-300 text-lg">Connect to GitHub</h3>
              </div>
              <p className="text-sm text-slate-500 mb-4">
                Provide a <a href="https://github.com/settings/tokens/new?scopes=repo,user" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">Personal Access Token</a> with `repo` and `user` scopes to save projects directly to your repositories.
              </p>
              <div className="flex flex-col md:flex-row gap-4 items-start">
                <input type="password" value={githubPat} onChange={e => setGithubPat(e.target.value)} placeholder="ghp_..." className="w-full p-3 bg-white/[0.05] border border-white/10 rounded-lg shadow-inner placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 flex-grow" />
                <button onClick={handleConnectGitHub} disabled={isConnecting} className="w-full md:w-auto px-5 py-3 font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-500 text-white transition-colors">
        ...
        <div className="mt-16 bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-2xl p-8 shadow-[0_0_120px_rgba(255,255,255,0.1)] animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                <div className="flex flex-col gap-2">
                    ...
                    <input id="secret-name" type="text" value={newSecretName} onChange={(e) => setNewSecretName(e.target.value)} placeholder="E.g., OPENAI_API_KEY" className="w-full p-3 bg-white/[0.05] border border-white/10 rounded-lg shadow-inner placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="secret-value" className="font-semibold text-sm text-slate-400">Secret Value</label>
                    <div className="relative"><input id="secret-value" type={isSecretValueVisible ? 'text' : 'password'} value={newSecretValue} onChange={(e) => setNewSecretValue(e.target.value)} placeholder="Enter secret value" className="w-full p-3 pr-12 bg-white/[0.05] border border-white/10 rounded-lg shadow-inner placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                        <button onClick={() => setIsSecretValueVisible(!isSecretValueVisible)} className="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 flex items-center justify-center text-slate-400 hover:text-white" aria-label={isSecretValueVisible ? 'Hide' : 'Show'}>
            ...
            {secrets.length > 0 && <div className="mt-8 border-t border-slate-700 pt-6"><h3 className="font-semibold text-slate-300 mb-4">Saved Secrets</h3><div className="space-y-3">
                {secrets.map(secret => { const tool = identifyTool(secret); const IconComponent = tool.icon; return (<div key={secret.name} className="flex items-center justify-between bg-slate-800/50 p-3 rounded-lg"><div className="flex items-center gap-3"><IconComponent className="w-6 h-6 text-slate-400" /><div><p className="font-mono text-sm text-slate-200">{secret.name}</p><p className="text-xs text-slate-500">{tool.name}</p></div></div><button onClick={() => handleRemoveSecret(secret.name)} className="p-2 text-slate-500 hover:text-red-400" aria-label={`Remove ${secret.name}`}><TrashIcon className="w-5 h-5" /></button></div>);})}
            </div></div>}
        </div>
